...
private:
    ...
    bool solve(size_t row, size_t col, std::set<int> &isZero); //Helper function for solve()
    bool solutionHasValidFrequencyAndRangeOfNumbers();
    void print();

    bool updateSetOfZeros(std::set<int> &isZero, int index);
    void updatePossibleValuesRowColBox(size_t row, size_t col, std::set<int>& possibleValues);
    void updatePossibleRowValues(size_t row,size_t col, std::set<int> &possibleValues);
    void updatePossibleColValues(size_t row, size_t col, std::set<int> &possibleValues);
    void updatePossibleColValues(size_t row, size_t col, std::set<int> &possibleValues, size_t boxWidth);
    void updatePossibleBoxValues(int boxStart_row, int boxStart_col, std::set<int> &possibleValues, int count);
    int  determineBoxStart(int loc);
